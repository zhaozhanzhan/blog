...
class FrontArticle extends Model
{
    ...
    public function getRelease($start = 0, $pageSize = 15)
    {
        ...
        $readingRanking = $this->readingRanking();
        return [
            "code" => Code::$SUCCESS_NO_TIP,
            "msg" => "success",
            "total" => $total,
    ...
    public function detail($id, $ip)
    {
        ...
        //点击量
        $red = Redis::zscore(RedisKey::$ARTICLE_READ_NUM, $ip . $id);
        if (!$red) {
            $res = FrontArticle::where(["id" => $id])
                ->update(["read_num" => $article[0]->read_num + 1]);
            if ($res) {
                Redis::zadd(RedisKey::$ARTICLE_READ_NUM, 1, $ip . $id);
...
//            ->orderBy('id', 'desc')
    ...
    public function upDownArticle($id)
    {
    ...
    {
        return FrontArticle::where(["release" => 1])
    ...
    public function thumb($id, $ip)
    {
        ...
        $red = Redis::zscore(RedisKey::$ARTICLE_THUMB_NUM, $ip . "." . $id);
        if ($red) {
            return ["code" => Code::$INFO, "msg" => "赞多了容易骄傲！"];
        }
        $res = $article->increment("thumb_num");
        if ($res) {
            Redis::zadd(RedisKey::$ARTICLE_THUMB_NUM, 1, $ip . "." . $id);
        }
     ...
     * @return array
     */
    public function currentTypeArticle($type)
    {
